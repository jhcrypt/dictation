...
def _fast_jarvis_match(lower):
    ...
    # Volume
    if any(p in lower for p in ("volume up", "turn up", "louder")):
        return ("set_volume", {"direction": "up"})
    if any(p in lower for p in ("volume down", "turn down", "quieter", "lower volume")):
        return ("set_volume", {"direction": "down"})
    if any(p in lower for p in ("mute", "silence")):
